...
export interface PJMovie {
  ...
  poster: string;
  type: 'movie' | 'series';
  source?: 'pj' | 'archive'; // pj = pelisjuanita (default), archive = archive.org
  embedUrl?: string;          // URL directa para archive.org
}
...
export const pjMovies: PJMovie[] = [
  ...
  // ── Archive.org — reproducción directa ─────────────────────────────────
  ...
  { id:'a5', slug:'lilo-stitch', title:'Lilo y Stitch',                   year:2002, genre:'Animación', imdb:'7.3', poster:'https://image.tmdb.org/t/p/w500/5B0a13CUKoAqWg0MbWMXalIFGEE.jpg', type:'movie', source:'archive', embedUrl:ARCHIVE_URL('f290279344_202603/f288495560.mp4') },
...
function EmbedModal({ item, onClose }: { item: PJMovie; onClose: () => void }) {
  ...
  const isArchive = item.source === 'archive';
    ...
    <div className="mv-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="mv-modal-footer">
          <span className="mv-footer-note">{isArchive ? "Contenido servido por Archive.org · Reproducción directa ✓" : "Contenido servido por pelisjuanita.com"}</span>
